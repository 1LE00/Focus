...
export const Settings = () => {
    const {
        minutes, setMinutes, setTracker
    } = useContext(TimerContext);

    const {
        autoStart, setAutoStart, darkTheme, setDarkTheme, toggle, setToggle,
        notificationOptions, setNotificationOptions, longBreakInterval, setLongBreakInterval
    ...
    const handleLongBreak = (event) => {
        const value = event.target.value;
        return value === '' ? setLongBreakInterval('') : setLongBreakInterval(Math.min(30, Number.parseInt(value)));
    };
    ...
    const handleNotificationsChange = (event) => {
        ...
        if (Notification.permission !== 'granted') {
            Notification.requestPermission().then(permission => {
                if (permission === 'granted') {
                    setNotificationOptions(previous => ({ ...previous, enabled: isChecked }));
                } else {
        ...
        if (!isChecked) {
            setNotificationOptions(previous => ({ ...previous, silent: isChecked }))
        }
    };
    return (
        <section className={`${toggle.settings ? 'flex' : 'hidden'} justify-center fixed inset-0 bg-black/75 z-10 overflow-auto`} onClick={closeModal}>
            <section className={`${toggle.settings ? 'flex' : 'hidden'} flex-col relative top-11 rounded-md m-3 p-4 w-modal-custom max-w-xs bg-slate-50 overflow-auto`} onClick={(event) => event.stopPropagation()}>
                <header className="text-gray-500 flex justify-between items-center w-full pb-2 border-b border-black/20">
                ...
                <section className="timer-options text-gray-500 my-4">
                    ...
                    <section className="flex items-center justify-between my-4">
                        ...
                        <input type="checkbox" name="auto-break" id="auto-break" className="w-14 h-8 rounded-3xl bg-gray-500/10 relative checked:bg-green-400 shadow-checkbox transition-colors ease-linear duration-500 cursor-pointer" aria-label="Set auto-focus to true" checked={autoStart.break} onChange={(e) => setAutoStart(prev => {
                            return {
                                ...prev,
                    ...
                    <section className="flex items-center justify-between mb-4">
                        ...
                        <input type="checkbox" name="auto-focus" id="auto-focus" className="w-14 h-8 rounded-3xl bg-gray-500/10 relative checked:bg-green-400 shadow-checkbox transition-colors ease-linear duration-500 cursor-pointer" aria-label="Set auto-break to true" checked={autoStart.focus} onChange={(e) => setAutoStart(prev => {
                            return {
                                ...prev,
                    ...
                    <section className="flex items-center justify-between pb-8 pt-2 border-b border-black/20">
                        ...
                        <input type="number" name="long-break-interval" id="long-break-interval" className="bg-gray-500/10 rounded text-center text-black/50 p-2 w-20 text-sm border-t-0 border-l-0 border-b-2 border-solid border-b-black/10 border-r-2 border-r-black/10 shadow-number" min={1} max={999} value={longBreakInterval} onChange={(e) => handleLongBreak(e)} onKeyDown={handleKeyDown} onBlur={() => setLongBreakInterval(value => {
                            return value.toString().replace(/^0+/, '') || 4;
                        })} />
                    </section>
                </section>
                {/* Timer options */}
                ...
                <section className="text-gray-500 my-4 border-b border-b-black/20">
                    ...
                    <section className="theme-options mt-4">
                        ...
                        <section className="flex items-center justify-between my-4">
                            ...
                            <input type="checkbox" name="dark-mode" id="dark-mode" className="w-14 h-8 rounded-3xl bg-gray-500/10 relative checked:bg-green-400 shadow-checkbox transition-colors ease-linear duration-500 cursor-pointer" aria-label="Set dark mode to true" checked={darkTheme} onChange={(e) => setDarkTheme(e.target.checked)} />
                        </section>
                    </section>
                </section>
                {/* Theme options */}
